...
func ReadLockfile() (*models.Lockfile, error) {
	...
	// Initialize nested maps and migrate old lockfiles for each namespace
	for name, entry := range lockfile.Namespaces {
		...
		// Auto-migrate older lockfiles that didn't track Bundles/ExplicitSkills.
		// Skip if SyncAll is true — it already covers all skills.
		if !entry.SyncAll && len(entry.Bundles) == 0 && len(entry.ExplicitSkills) == 0 && len(entry.Skills.Installed) > 0 {
			var migratedSkills []string
			for skillName := range entry.Skills.Installed {
				migratedSkills = append(migratedSkills, skillName)
			}
			entry.ExplicitSkills = migratedSkills
		}
