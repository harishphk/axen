...
func TestFindSkillDirsDepthLimit(t *testing.T) {
	tempDir := t.TempDir()

	// Create a skill at depth 3 (should be found)
	// rootDir -> level1 -> level2 -> level3 -> SKILL.md
	depth3Dir := filepath.Join(tempDir, "l1", "l2", "l3")
	_ = os.MkdirAll(depth3Dir, 0755)
	_ = os.WriteFile(filepath.Join(depth3Dir, "SKILL.md"), []byte(""), 0644)

	// Create a skill at depth 5 (should NOT be found because maxDepth is 4)
	// rootDir -> level1 -> level2 -> level3 -> l4 -> l5 -> SKILL.md
	depth5Dir := filepath.Join(tempDir, "l1", "l2", "l3", "l4", "l5")
	_ = os.MkdirAll(depth5Dir, 0755)
	_ = os.WriteFile(filepath.Join(depth5Dir, "SKILL.md"), []byte(""), 0644)
	...
	if !foundDepth3 {
		t.Error("Expected to find skill directory at depth 3, but did not")
	}
	if foundDepth5 {
		t.Error("Expected NOT to find skill directory at depth 5, but did")
	}
}
