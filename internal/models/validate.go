...
// skillNameRegex matches lowercase alphanumeric names with hyphens and underscores.
var skillNameRegex = regexp.MustCompile(`^[a-z0-9](?:[a-z0-9-_]*[a-z0-9])?$`)
...
func ValidateManifest(m *Manifest) error {
	...
	for skillName := range m.Skills {
		if !skillNameRegex.MatchString(skillName) {
			return fmt.Errorf("manifest: skill name %q must be lowercase kebab-case (a-z, 0-9, hyphens)", skillName)
		}
	}

	hasDefaultBundle := false
	for bundleName, bundle := range m.Bundles {
		if bundle.IsDefault {
			if hasDefaultBundle {
				return fmt.Errorf("manifest: multiple default bundles defined")
			}
			hasDefaultBundle = true
		}
		for _, skillName := range bundle.Skills {
			if _, exists := m.Skills[skillName]; !exists {
				return fmt.Errorf("manifest: bundle %q references missing skill %q", bundleName, skillName)
			}
		}
	}
