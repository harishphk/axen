...
We follow the **Conventional Commits** specification for commit messages and Pull Request titles. This helps automate release notes and changelogs.

> [!IMPORTANT]
> This format is strictly enforced in our CI pipeline. Both **Pull Request titles** (during PR workflows) and **commit messages** (on direct pushes) must match the Conventional Commits regex, or the build status will fail.
...
*   `chore`: Tooling updates or general housekeeping.
